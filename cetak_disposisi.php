...
if (empty($_SESSION['admin'])) {
    ...
    if (mysqli_num_rows($query) > 0) {
            ...
            echo '
                    <table class="bordered" id="tbl">
                        <tbody>
                            ...
                            <tr>
                                <td id="right" width="18%"><strong>Nama PIC</strong></td>
        ...
        echo '
            ...
            <div id="lead">
                <p>Manager Aset Properti Dan Umum</p>
                <div style="height: 50px;"></div>';
        $query = mysqli_query($config, "SELECT manager, nip FROM tbl_instansi");
        list($manager, $nip) = mysqli_fetch_array($query);
        if (!empty($manager)) {
            echo '<p class="lead">' . $manager . '</p>';
        } else {
            echo '<p class="lead">Riska Adrianti</p>';
        }
        if (!empty($nip)) {
            echo '<p>NIP. ' . $nip . '</p>';
        } else {
