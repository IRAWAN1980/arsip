    ...
    if ($_SESSION['admin'] != 1 and $_SESSION['admin'] != 2) {
        ...
        if (isset($_REQUEST['sub'])) {
            ...
            if (isset($_REQUEST['submit'])) {
                ...
                if (
                    $_REQUEST['institusi'] == "" || $_REQUEST['nama'] == "" || $_REQUEST['status'] == "" || $_REQUEST['alamat'] == "" || $_REQUEST['manager'] == "" || $_REQUEST['nip'] == ""
                    ...
                    if (!preg_match("/^[a-zA-Z0-9. -]*$/", $nama)) {
                        ...
                        if (!preg_match("/^[a-zA-Z0-9. -]*$/", $institusi)) {
                            $_SESSION['institusi'] = 'Form Nama Sub Bidang  hanya boleh mengandung karakter huruf, angka, spasi, titik(.) dan minus(-)';
                            echo '<script language="javascript">window.history.back();</script>';
                        } else {
                            ...
                            if (!preg_match("/^[a-zA-Z0-9.,:\/<> -\"]*$/", $status)) {
                                ...
                                if (!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $alamat)) {
                                    ...
                                    if (!preg_match("/^[a-zA-Z., ]*$/", $manager)) {
                                        $_SESSION['manager'] = 'Form Nama Manager Aset Properti dan Umum  hanya boleh mengandung karakter huruf, spasi, titik(.) dan koma(,)<br/><br/>';
                                        echo '<script language="javascript">window.history.back();</script>';
                                    } else {

                                        if (!preg_match("/^[0-9 -]*$/", $nip)) {
                                            $_SESSION['nipmanager'] = 'Form NIP Manager Aset Properti Dan Umum hanya boleh mengandung karakter angka, spasi, dan minus(-)<br/><br/>';
                                            echo '<script language="javascript">window.history.back();</script>';
                                        } else {
                                                    ...
                                                    if (!empty($logo)) {
                                                        ...
                                                        if (in_array($eks, $ekstensi) == true) {
                                                            if ($ukuran < 2000000) {
                                                                ...
                                                                $query = mysqli_query($config, "UPDATE tbl_instansi SET institusi='$institusi',nama='$nama',status='$status',alamat='$alamat',manager='$manager',nip='$nip',website='$website',email='$email',logo='$nlogo',id_user='$id_user' WHERE id_instansi='$id_instansi'");
                                                        ...
                                                        $query = mysqli_query($config, "UPDATE tbl_instansi SET institusi='$institusi',nama='$nama',status='$status',alamat='$alamat',manager='$manager',nip='$nip',website='$website',email='$email',id_user='$id_user' WHERE id_instansi='$id_instansi'");
                            ...
                            <form class="col s12" method="post" action="?page=sett" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    ...
                                    <div class="input-field col s6">
                                        ...
                                        ?>
                                        <label for="institusi">Nama Sub Bidang</label>
                                    </div>
                                    <div class="input-field col s6">
                                    ...
                                    <div class="input-field col s6">
                                        ...
                                        <input id="manager" type="text" class="validate" name="manager" value="<?php echo $row['manager']; ?>"
                                            required>
                                        <?php
                                        if (isset($_SESSION['manager'])) {
                                            $manager = $_SESSION['manager'];
                                            echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">' . $manager . '</div>';
                                            unset($_SESSION['manager']);
                                        }
                                        ?>
                                        <label for="manager">Nama Manager Aset Properti dan umum</label>
                                    </div>
                                    <div class="input-field col s6">
                                    ...
                                    <div class="input-field col s6">
                                        ...
                                        <?php
                                        if (isset($_SESSION['nipmanager'])) {
                                            $nipmanager = $_SESSION['nipmanager'];
                                            echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">' . $nipmanager . '</div>';
                                            unset($_SESSION['nipmanager']);
                                        }
                                        ?>
                                        <label for="nip">NIP Manager Aset Properti Dan Umum</label>
                                    </div>
                                    <div class="input-field col s6">
